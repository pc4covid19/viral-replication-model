...
#include "./viral_replication_submodel.h" 


using namespace PhysiCell; 

std::string viral_replication_model_version = "0.1.0"; 

Submodel_Information viral_replication_submodel_info; 

void viral_replication_submodel_setup( void )
{
		// set version 
	viral_replication_submodel_info.name = "viral replication model"; 
	viral_replication_submodel_info.version = viral_replication_model_version; 

		// set functions 
	viral_replication_submodel_info.main_function = viral_replication_main_model;
	viral_replication_submodel_info.phenotype_function = NULL; // pushed into the "main" model  
	viral_replication_submodel_info.mechanics_function = NULL;

		// what microenvironment variables do you need 
	viral_replication_submodel_info.microenvironment_variables.push_back( "virion" );

		// what cell variables and parameters do you need? 
	viral_replication_submodel_info.cell_variables.push_back( "unbound_external_ACE2" ); 
	
	viral_replication_submodel_info.register_model(); 
...
{
	std::cout << "---------- viral_replication_main_model: " << PhysiCell_globals.current_time << std::endl;
	#pragma omp parallel for 
	for( int n=0; n < (*all_cells).size() ; n++ )
	{
		Cell* pC = (*all_cells)[n]; 
		if( pC->phenotype.death.dead == false )
		{ viral_replication_model( pC, pC->phenotype , dt ); }
	}
